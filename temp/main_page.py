...
def main_page():
    ...
    # Full block for handling delete functionality
    if "delete_id" in st.query_params:
        delete_id = st.query_params["delete_id"]  # Get the delete_id directly
        delete_pez_item(delete_id)  # Perform the delete operation
        st.success(f"PEZ item with ID {delete_id} deleted successfully!")  # Confirm deletion

        # Clear query parameters and refresh page
        st.query_params.clear()
        st.rerun()  # Reload the page to refresh the item list
